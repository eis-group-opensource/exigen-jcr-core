...
public class BaseNamespaceRegistryImpl extends AbstractNamespaceResolver
    ...
    /** this is a global object accessed from multiple threads concurrently: don't try to write to it when somebody might be reading */
    ...
    /** only replace namespaces field when it's ready: some threads might be reading it, so clearing it causes concurency problem, as we might be pulling the stool out from some other thread's feet. */
	public void loadNamespaces(DatabaseConnection conn)
			throws RepositoryException {
		// TODO load all namespaces
    	ArrayList<Namespace> loading_namespaces = new ArrayList<Namespace>();
		...
		while (st.hasNext()) {
			...
			addNamespace(loading_namespaces, id, prefix, uri);
		}
		st.close();

		addNamespace(loading_namespaces, null, "", "");
        namespaces = loading_namespaces; // it's loaded: push it out live
	}

	protected void addNamespace(ArrayList<Namespace> namespaces, Long id, String prefix, String uri) {
		if (prefix.equals("")) {
			for (Namespace n : namespaces) {
	...
	protected void addNamespace(Long id, String prefix, String uri) {
		addNamespace( namespaces, id, prefix, uri);
	}
