...
//            + "([^ 1234567890"+EXCLUDE_PART+".\\-/:\\[\\]*'\"|](?:[^"+EXCLUDE_PART+"/:\\[\\]*'\"|]*[^"+EXCLUDE_PART+" /:\\[\\]*'\"|])?)"
    ...
    private static final SoftHashMap<Path, ConcurrentHashMap<QName, Path>> pathCahe = new SoftHashMap<Path, ConcurrentHashMap<QName,Path>>();
    ...
    public static Path create(Path parent, QName name, boolean normalize)
    	...
    	ConcurrentHashMap<QName, Path> cache = pathCahe.get(parent);
    	if (cache != null){
        ...
        if (cache == null){
        	cache = new ConcurrentHashMap<QName, Path>();
